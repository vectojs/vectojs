...
/**
 * Lay out a run of text against MSDF font metrics.
 *
 * Pure: it reads only its arguments and touches no DOM, canvas, or worker API.
 * That is deliberate and load-bearing — the same function runs inside the layout
 * worker (see `LayoutWorker.ts`, which is bundled into a Blob) *and*
 * synchronously on the main thread when no worker is available, so a
 * worker-hostile environment degrades to identical geometry instead of
 * permanently invisible text. Keep it free of environment access: anything
 * added here that depends on `self`, `window`, or a canvas silently breaks one
 * of the two callers.
 *
 * Time: O(n) in `request.text` length, plus O(n) for the optional justify pass.
 * Space: O(n) for the four output arrays.
 */
export function computeMSDFLayout(
