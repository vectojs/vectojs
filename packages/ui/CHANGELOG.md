...
- b3803fc: Fix `Dropdown` menu options being unselectable with a real pointer click.
...
- f145c51: Fix `Input` projecting a `line-height` smaller than its own font, which crushed
...
- f700a54: Markdown gains an opt-in `virtualize` option that materializes only the
...
- beed405: fix(ui): line.font now carries bold/italic for single-style lines (GH-459)
...
- 2c0d682: fix: extend per-grapheme carriers to RichText, so Markdown body text stops drifting on Firefox
...
- 30d809a: Stop leaking a measuring canvas per `RichText`
