...
import { SpringPhysics } from '../src/SpringPhysics';
...
describe('SpringPhysics', () => {
  ...
  describe('Stress & Zero-GC validation', () => {
    it('updates 50,000 springs in one frame, all finite and advancing toward target', () => {
      const count = 50000;
      ...
      // Report benchmark time as a signal only. Wall-clock micro-timings are
      // shared-CPU-noisy under a full parallel test run, so correctness — not a
      // hard millisecond gate — is what this test asserts. Track timing via the
