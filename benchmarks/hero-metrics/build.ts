...
// page/index.html. Run before ../run-browsers.sh hero-metrics <port>.
//   bun run benchmarks/hero-metrics/build.ts
  ...
  plugins: [
    {
      name: 'vecto-src',
      setup(b) {
        b.onResolve({ filter: /^@vectojs\/devtools\/headless$/ }, () => ({
          path: join(PKGS, 'devtools/src/headless.ts'),
        }));
        b.onResolve(
          { filter: /^@vectojs\/(core|text|layout|math|animation|ui|devtools)$/ },
          (a) => ({
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>vectojs hero-metrics gate bench</title>
  </head>
  <body>
