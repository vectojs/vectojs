...
2. **Build via `just wasm`** (which runs the crate's `build.sh`), or replicate its exact `RUSTFLAGS` env-var override if scripting something new:

   ```bash
   just wasm   # preferred — runs crates/vectojs-core-rs/build.sh with correct RUSTFLAGS

   # equivalent raw form (what build.sh does):
...
5. **Format and lint before considering a Rust change complete**: `just wasm-check` (runs `cargo fmt --check` + `cargo clippy --target wasm32-unknown-unknown -- -D warnings` with the correct RUSTFLAGS). Both must pass. Use `just wasm-test` to build the wasm and run the core differential suite against it.
