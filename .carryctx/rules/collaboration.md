...
6. Use workspace `tmp/` for scratch in the current task (or `carryctx worktree` for isolation); do not drop temp files in package directories or system `/tmp`. `references/` clones are read-only — never edited or committed.
