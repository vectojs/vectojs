...
class PhysicsTextbookEntity extends Entity {
  ...
  constructor(text: string, atlas: any) {
    ...
    const res = this.layoutEngine.layoutText(text, atlas, this.fontSize);
  ...
  render(renderer: any) {
    ...
    // 背景纸张颜色
    renderer.fillStyle = '#fdf6e3';
    renderer.fillRect(0, 0, window.innerWidth, window.innerHeight);

    // 绘制弹性连接线 (展示物理形变)
    renderer.strokeStyle = 'rgba(203, 75, 22, 0.2)';
    ...
    for (const c of this.chars) {
      ...
      // Hooke's Law 标题加粗加红
      if (c.targetY < 150) {
        renderer.fillText(c.char, 0, 0, `bold ${c.fontSize}px serif`, '#cb4b16');
      } else {
        renderer.fillText(c.char, 0, 0, `${c.fontSize}px serif`, '#657b83');
      }
      renderer.restore();
    }
