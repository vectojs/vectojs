...
export default async function getCommitMessages(settings) {
    ...
    if (!from && fromLastTag) {
        const output = await x("git", ["describe", "--abbrev=40", "--always", "--first-parent", "--long", "--tags"], { nodeOptions: { cwd } });
    ...
    if (from) {
        ...
        if (mergeBase.exitCode === 1) {
            throw new Error(`Cannot find merge-base between '${from}' and '${effectiveTo}'. ` +
