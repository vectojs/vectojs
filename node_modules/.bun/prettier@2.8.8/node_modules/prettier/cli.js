    ...
    module2.exports = function(args, opts) {
      ...
      for (var i = 0; i < args.length; i++) {
        ...
        if (/^--.+=/.test(arg)) {
          ...
          var key = arg.match(/^--(.+)/)[1];
          var next = args[i + 1];
          if (next !== void 0 && !next.startsWith('-') && !flags.bools[key] && !flags.allBools && (aliases[key] ? !aliasIsBoolean(key) : true)) {
            setArg(key, next, arg);
            i++;
          ...
          if (oldRangeStart) {
            if (lines.length <= options.context * 2 && i2 < diff.length - 2) {
              ...
              if (i2 >= diff.length - 2 && lines.length <= options.context) {
                var oldEOFNewline = oldStr.endsWith('\n');
                var newEOFNewline = newStr.endsWith('\n');
