...
- **Local CI dry-run**: `act` (Go binary `~/.local/share/go/bin/act`, version 0.2.89, never via `npx`/`bunx` — §1 Tool Invocation) — validate workflows without running containers: `act -n -W .github/workflows/ci.yml` (dry-run, checks syntax and job graph) or `act -n` for all workflows. `codeql`/`gitleaks`/`trivy` provide the CodeQL/security gate locally where available (`which codeql`, `gitleaks detect --source .`); otherwise CI enforces them. Use these before declaring a PR ready, alongside `just check`.
...
5. **No Pollution**: Do not write temporary files or scratchpads into the package directories. Use `$VECTOJS_WORKSPACE/recording/` for scratch files (never `tmp/`, never system `/tmp`).
6. **Task management via CarryCtx**: `.carryctx/` holds project config, `rules/`, `workflows/`, and `personas/` presets — check the relevant file before starting domain-specific work. Always pass `--agent <name>` (registered names: `omp`, `opencode`, `claude-code`, `kiro`, `codex`) and use the locked local CLI through `bunx carryctx`. Use `progress note/decision add/checkpoint` to record findings; close or block every task at session end. Before the first commit, confirm `bunx carryctx hooks status`: this repository deliberately enables only the CarryCtx `post-commit` checkpoint hook, because its task-ID message prefix is incompatible with Commitlint. Follow `.carryctx/workflows/change-lifecycle.md` for planned changes.
...
9. **Post-wave review gate (release-blocking)**: before tagging or publishing any release, run a delta code review of everything merged since the last release — `git diff <last-release-tag>..origin/main`, every hunk read against current source, hunting regressions the merged fixes themselves introduced (twin paths missed, interactions between concurrently-landed PRs, tests that would pass on unfixed code). Confirmed defects become issues prefixed `r<N>(<area>):` with mechanism + `file:line` + introducing PR and are **fixed before publishing**; the loop repeats until a review round returns clean. P3/P4 findings may ride the next per-package backlog issue instead of blocking. Verify build claims only in a fresh `git worktree add` of `origin/main` — long-lived checkouts go stale and have produced false "broken main" alarms. (Instituted 2026-08-24 after review rounds 1–3 of the full-monorepo sweep.)

10. **File size and delegation**: Prefer small focused modules over a single growing file. When a file exceeds ~800 lines or mixes two domains (e.g. `Scene.ts` 6.5k lines spanning lifecycle, hit-test, resize, a11y), propose a split and delegate the work. The commander decomposes the effort into tasks with explicit file scopes and dependencies (`carryctx task create --team <name> --depends-on CTX-NNNN`, `task scope add`), dispatches implementation to subagents in isolated worktrees (`carryctx worktree create CTX-NNNN`), and verifies via `team status`/`team context`/`task show` rather than trusting self-reports. Keep your own context lean (outline before reading) and never leave a task `in_progress` after its agent stops.
