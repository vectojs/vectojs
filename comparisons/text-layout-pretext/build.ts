...
// page/index.html. Run before ../run-browsers.sh text-layout-pretext <port>.
//   bun run benchmarks/text-layout-pretext/build.ts
import { dirname, join, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

const HERE = dirname(fileURLToPath(import.meta.url));
const PKGS = resolve(HERE, '../../packages');
// `@chenglou/pretext` is a real dependency of this comparison dir; Bun resolves
// it from ./node_modules without help. Only @vectojs/* needs source aliasing.
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>vectojs text-layout-pretext gate bench</title>
  </head>
  <body>
